...
async function fetchSerieById() {
  try {
    ...
    const response = await fetch(
      `https://api.themoviedb.org/3/tv/${serieId}?api_key=${import.meta.env.VITE_TMBD_API_KEY}&language=pt-BR`
    )
...
<template>
  ...
  <section
    ...
    :style="{
      backgroundImage: `linear-gradient(to right, black, rgba(0, 0, 0, 0.5)), url(${serie?.backdrop_path})`
    }"
  >
    <div
